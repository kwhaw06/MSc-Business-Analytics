...
# fit the lda model
ldaFit=train(train_rand[,-5],   # features
             train_rand[,5],    # labels
             method="lda",      # if method is "lda" then cant tune dimension
             trControl=trainControl(method = "none"))  # nothing to tune so this is "none"

ldaFit$finalModel   # obtain same output as Iris LDA
...
lda2Fit=train(train_rand[,-5],train_rand[,5],
              method="lda2",   # "lda2" allows us to tune the dimension
              trControl=fitControl)
plot(lda2Fit)

lda2Fit   # tells us the best model is when we use 1 dimension

lda2Fit$finalModel   # same output as using "lda" cuz this doesn't specify the final model 
